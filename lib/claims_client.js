...
Object.defineProperties(ClaimsClient.prototype, {
	...
	expand: {
		...
		value: function expand(claims, csid, cid, callback) {
			...
			var path = '/claims/'.concat(claims.userId);
			if ('undefined' === typeof csid) {
				path += '/claimset/'.concat(csid);
				if ('undefined' === typeof cid) {
					path += '/claim/'.concat(cid);
					this._log('info', 'claims_client.expand -- arity3: expanding claim ['.concat(cid, ']', 'from claimset [', csid, ']'));
				} else {
					this._log('info', 'claims_client.expand -- arity2: expanding claimset ['.concat(csid, ']'));	
				}
			} else {
